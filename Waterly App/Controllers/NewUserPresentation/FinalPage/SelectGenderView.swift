...
final class SelectGenderView: BaseView {
    
    let titleLabel: UILabel = {
        let label = UILabel()
        label.font = Resources.Fonts.sfProRegular(size: 20)
        label.text = Resources.Strings.NewUserPageVC.FinalPage.genderTitle
        label.textColor = .white
        return label
    }()
    
    let maleRadioButton = RadioButton(.selected, color: .white)
    
    let maleLabel: UILabel = {
        let label = UILabel()
        label.font = Resources.Fonts.sfProRegular(size: 16)
        label.text = Resources.Strings.NewUserPageVC.FinalPage.male
        label.textColor = .white
        return label
    }()
    
    let femaleRadioButton = RadioButton(.normal, color: .white)
    
    let femaleLabel: UILabel = {
        let label = UILabel()
        label.font = Resources.Fonts.sfProRegular(size: 16)
        label.text = Resources.Strings.NewUserPageVC.FinalPage.female
        label.textColor = .white
        return label
    }()
}
